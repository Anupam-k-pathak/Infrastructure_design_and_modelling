    ...
    with open(file_name, "r+")as f:
        ...
        for i in range(len(data)):
            ...
            if (float(data[i].split(",")[0]) / 1000000) < 151:
                if (float(NW_initiate_unit_buffer3)) > max_buffer_fill:
                    max_buffer_fill = float(NW_initiate_unit_buffer3)
...
def priority_distribution_count(file_name1, file_name2, time_variable, mp, exp_name, abs_path, max_time):
    ...
    print(mp + "," + str((transaction_sum1 + transaction_sum2)*256/max_time))
    x_axis = []
    y_axis = []
